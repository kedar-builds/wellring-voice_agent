"""
test_cases.py
=============
10 realistic health scenarios to validate the scoring and alert engine.

Run directly with:
    python -m src.scoring_engine.test_cases

Each test case includes:
    - symptoms      : list of symptom identifiers
    - severity      : "low" | "medium" | "high" | "critical"
    - description   : plain-English description of the scenario
    - expected_level: the RiskLevel the test must produce to pass
"""

import sys
import os

# Allow running as a standalone script from the repo root
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", ".."))

from src.scoring_engine.scoring import calculate_score
from src.scoring_engine.alerts  import determine_action
from src.scoring_engine.baseline import RiskLevel

# ---------------------------------------------------------------------------
# Test case definitions
# ---------------------------------------------------------------------------

TEST_CASES = [
    # ── Case 1: Single mild symptom, low severity ──────────────────────────
    {
        "id":             1,
        "description":    "User reports mild dizziness, nothing else.",
        "symptoms":       ["dizziness"],
        "severity":       "low",
        "expected_level": RiskLevel.LOW,        # score = 20
    },
    # ── Case 2: Missed medicine only ──────────────────────────────────────
    {
        "id":             2,
        "description":    "User forgot to take medicine this morning.",
        "symptoms":       ["medicine_missed"],
        "severity":       "low",
        "expected_level": RiskLevel.LOW,        # score = 10
    },
    # ── Case 3: Fever + dizziness, medium severity ────────────────────────
    {
        "id":             3,
        "description":    "User has a fever and feels dizzy.",
        "symptoms":       ["fever", "dizziness"],
        "severity":       "medium",
        "expected_level": RiskLevel.MEDIUM,     # score = 15+20+10 = 45
    },
    # ── Case 4: Medicine missed + fever, medium severity ─────────────────
    {
        "id":             4,
        "description":    "User missed medicine and has a fever.",
        "symptoms":       ["medicine_missed", "fever"],
        "severity":       "medium",
        "expected_level": RiskLevel.MEDIUM,     # score = 10+15+10 = 35
    },
    # ── Case 5: Breathing problem, high severity ──────────────────────────
    {
        "id":             5,
        "description":    "User is struggling to breathe after climbing stairs.",
        "symptoms":       ["breathing_problem"],
        "severity":       "high",
        "expected_level": RiskLevel.HIGH,       # score = 40+20 = 60 → MEDIUM boundary
        # NOTE: 60 falls in MEDIUM (31-60). Adjust expected level accordingly.
        "expected_level": RiskLevel.MEDIUM,
    },
    # ── Case 6: Fall detected, medium severity ────────────────────────────
    {
        "id":             6,
        "description":    "Sensor detected a fall; user says they are okay.",
        "symptoms":       ["fall_detected"],
        "severity":       "medium",
        "expected_level": RiskLevel.HIGH,       # score = 60+10 = 70
    },
    # ── Case 7: Chest pain, high severity ────────────────────────────────
    {
        "id":             7,
        "description":    "User reports chest pain and left-arm numbness (high severity).",
        "symptoms":       ["chest_pain"],
        "severity":       "high",
        "expected_level": RiskLevel.HIGH,       # score = 50+20 = 70
    },
    # ── Case 8: Chest pain + breathing problem, high severity ─────────────
    {
        "id":             8,
        "description":    "User has chest pain and difficulty breathing.",
        "symptoms":       ["chest_pain", "breathing_problem"],
        "severity":       "high",
        "expected_level": RiskLevel.CRITICAL,   # score = 50+40+20 = 110
    },
    # ── Case 9: Stroke symptoms, critical severity ────────────────────────
    {
        "id":             9,
        "description":    "User shows classic stroke symptoms (face drooping, slurred speech).",
        "symptoms":       ["stroke_symptoms"],
        "severity":       "critical",
        "expected_level": RiskLevel.CRITICAL,   # score = 100+40 = 140
    },
    # ── Case 10: Multiple critical symptoms ──────────────────────────────
    {
        "id":             10,
        "description":    "User is unconscious with stroke symptoms and fall detected.",
        "symptoms":       ["unconscious", "stroke_symptoms", "fall_detected"],
        "severity":       "critical",
        "expected_level": RiskLevel.CRITICAL,   # score = 100+100+60+40 = 300
    },
]

# ---------------------------------------------------------------------------
# Test runner
# ---------------------------------------------------------------------------

def run_tests() -> None:
    """Execute all test cases and print a pass/fail summary."""
    passed = 0
    failed = 0

    print("=" * 60)
    print("  WellRing — Scoring Engine Test Suite")
    print("=" * 60)

    for case in TEST_CASES:
        result     = calculate_score(case["symptoms"], case["severity"])
        alert      = determine_action(result["score"])

        actual_lvl = result["risk_level"]
        expect_lvl = case["expected_level"].value
        ok         = actual_lvl == expect_lvl

        status = "✅ PASS" if ok else "❌ FAIL"
        if ok:
            passed += 1
        else:
            failed += 1

        print(f"\nCase {case['id']:02d}: {status}")
        print(f"  Scenario  : {case['description']}")
        print(f"  Symptoms  : {case['symptoms']}")
        print(f"  Severity  : {case['severity']}")
        print(f"  Score     : {result['score']}")
        print(f"  Expected  : {expect_lvl}")
        print(f"  Got       : {actual_lvl}")
        print(f"  Action    : {alert['action']}")
        print(f"  Message   : {alert['message']}")

    print("\n" + "=" * 60)
    print(f"  Results: {passed} passed, {failed} failed out of {len(TEST_CASES)} tests")
    print("=" * 60)

    if failed > 0:
        sys.exit(1)


if __name__ == "__main__":
    run_tests()

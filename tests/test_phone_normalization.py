...
def _seed_user(db_path: str, user_id: str, name: str, phone: str, clerk_id=None):
    conn = sqlite3.connect(db_path)
    conn.execute(
        "INSERT INTO users (user_id, name, phone, role, clerk_id, created_at) "
        "VALUES (?, ?, ?, 'elderly', ?, '2026-01-01')",
        (user_id, name, phone, clerk_id),
    )
    conn.commit()
    conn.close()
...
def test_duplicate_phone_prefers_onboarded_profile(tmp_path):
    """When two users share a phone, the clerk_id profile wins the lookup
    (the dashboard is bound to it) over a clerk_id-less test row."""
    db_path = _make_db(str(tmp_path))
    # Test row created LAST (would win with plain created_at DESC)
    _seed_user(db_path, "test-row", "Test User", "+919004261186", clerk_id=None)
    # Real onboarded profile created EARLIER
    _seed_user(db_path, "real-user", "Mr. Sharma", "+919004261186", clerk_id="demo_sharma_001")
